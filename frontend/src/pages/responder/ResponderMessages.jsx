  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Chat with citizens</p>
      </div>
      ...
      <Card className="h-[600px] flex flex-col">
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.sender === 'responder' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${msg.sender === 'responder' ? 'bg-navy-800 text-white' : 'bg-gray-100 text-gray-900'}`}>
                  <p className="text-sm">{msg.text}</p>
                  <p className={`text-xs mt-1 ${msg.sender === 'responder' ? 'text-navy-200' : 'text-gray-500'}`}>{formatTimeAgo(msg.timestamp)}</p>
                </div>
              </div>
            ))
          )}
